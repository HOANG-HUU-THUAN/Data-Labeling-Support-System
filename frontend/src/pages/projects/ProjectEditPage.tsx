 ...
 * Form chỉnh sửa thông tin dự án hiện tại
 * 
 * Chức năng:
 * - Load dữ liệu dự án từ store theo projectId
 * - Cho phép chỉnh sửa: name, description, type
 * - Validation cơ bản
 * - Gọi API cập nhật dự án
 * - Navigate về trang chi tiết dự án sau khi cập nhật thành công
 */
const ProjectEditPage: React.FC = () => {
  ...
  /**
   * Effect: Khởi tạo form với dữ liệu dự án hiện tại
   */
  useEffect(() => {
    if (currentProject && !isInitialized) {
      setFormData({
        ...
        type: currentProject.type || ProjectType.OBJECT_DETECTION,
      });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (projectId) {
        ...
        setSuccessMessage(`Dự án "${formData.name}" đã được cập nhật thành công!`);
    ...
    return (
      <Container maxWidth="sm" sx={{ py: 4 }}>
        <Alert severity="error" onClose={clearError} sx={{ mb: 2 }}>
          {error || 'Không tìm thấy dự án để chỉnh sửa'}
        </Alert>
    ...
    <Container maxWidth="sm" sx={{ py: 4 }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Cập nhật thông tin cho dự án "{currentProject.name}"
        </Typography>
      </Box>
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary">
          ID dự án: <strong>{currentProject.id}</strong>
        </Typography>
      </Box>
    </Container>
  );
